'use strict';
factFindApp.controller('nonSourcePropertyDetailsCtrl', ['$scope', '$rootScope', 'propertyService', 'extraLenderService', 'quoteService',
    'firebaseService', 'localStorageService', '$firebaseObject', 'toaster', '$state',
    function ($scope, $rootScope, propertyService, extraLenderService, quoteService, firebaseService, localStorageService, $firebaseObject,
        toaster, $state) {
        ...
        $scope.checkPartnerTyping = function (field) {
            var flg = false;
            if($scope.typing.client && $scope.typing.client['nonsourcePropertyForm'] &&
                $scope.typing.client['nonsourcePropertyForm']['field'] == field &&
                $scope.typing.client['nonsourcePropertyForm']['status']) {
                flg = true;
            }
            return flg;
        };
        ...
        $scope.savePropertyForm = function (data) {
            propertyService.saveDetail($scope.customer.Properties).then(function (response) {
                ...
                extraLenderService.saveDetail($scope.customer.ExtraCommonLenderQn).then(function (response) {
                    ...
                    //                    });
                    //                    $scope.$parent.notification('Notice', 'Your Application is successfully filled in. Our Consultant will be in touch with you shortly.', 'Log Out', '', '', function (response) {
                    //                        if (response === 'Log Out') {
                    //                            $scope.$parent.logOut();
                    //                        }
                    //                    });
