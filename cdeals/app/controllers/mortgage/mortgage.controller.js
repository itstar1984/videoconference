...
factFindApp.controller('mortgageCtrl', ['$scope', '$rootScope', 'selectBoxService', 'extraLenderService', 'mortgageService', 'localStorageService',
    ...
    function ($scope, $rootScope, selectBoxService, extraLenderService, mortgageService, localStorageService, $firebaseObject,
        ...
        $scope.checkPartnerTyping = function (field) {
            var flg = false;
            if($scope.typing.consultant && $scope.typing.consultant['mortgageForm'] &&
                $scope.typing.consultant['mortgageForm']['field'] == field &&
                $scope.typing.consultant['mortgageForm']['status']) {
                flg = true;
            }
            return flg;
        };
        ...
        $scope.savePurchase = function (details) {
            ...
            MortAppDetail.MortAmtRequired = MortAppDetail.PropertyValue - (MortAppDetail.Deposit || 0) + (MortAppDetail.CapitalRaising ||
                0);
        ...
        $scope.getLTV = function (RemainingBalance, ERC, PropertyValue) {
            if(RemainingBalance && ERC && PropertyValue) {
                var totalAmount = parseFloat(RemainingBalance) + parseFloat(ERC);
                return(parseFloat(totalAmount / parseInt(PropertyValue)) * 100).toFixed(2);
            } else if(!RemainingBalance) {
                return(parseFloat(parseFloat(ERC) / parseFloat(PropertyValue)) * 100).toFixed(2);
            }
        };
        ...
        $scope.calculateLTVforPurchaseRTB = function () {
            if(
                $scope.customer.MortAppDetail.PropertyValue &&
                $scope.customer.MortAppDetail.RTBPurchasePriceAfterDiscount
            ) {
                var priceDisc = +$scope.customer.MortAppDetail.RTBPurchasePriceAfterDiscount || 0,
                    dep = +$scope.customer.MortAppDetail.Deposit || 0,
                    cap = +$scope.customer.MortAppDetail.CapitalRaising || 0;

                var loanRequiredforPurchaseSharedOwnership = Math.round(priceDisc - dep + cap);
                var ltvforPurchaseSharedOwnership = (loanRequiredforPurchaseSharedOwnership / $scope.customer.MortAppDetail.PropertyValue);
                return(ltvforPurchaseSharedOwnership * 100).toFixed(2);
            } else {
                return " ";
        ...
        $scope.calculateLaonRequiredforPurchaseRTB = function () {
            if($scope.customer.MortAppDetail.RTBPurchasePriceAfterDiscount) {
                var priceDisc = +$scope.customer.MortAppDetail.RTBPurchasePriceAfterDiscount || 0,
                    dep = +$scope.customer.MortAppDetail.Deposit || 0,
                    cap = +$scope.customer.MortAppDetail.CapitalRaising || 0;

                return(priceDisc - dep + cap).toFixed(2);
            } else {
                return " ";
            }
        };
