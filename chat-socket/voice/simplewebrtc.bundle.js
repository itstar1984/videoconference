            ...
            1: [ function ( require, module, exports ) {
                ...
                function SimpleWebRTC( opts ) {
                    ...
                    connection.on ( 'remove', function ( room ) {
                        //  if(room.conf && )
                        console.log ( room.id, self.connection.socket.sessionid )
                    ...
                    connection.on ( 'mute', function ( room ) {

                        if ( room.id == self.connection.socket.sessionid ) {
                            //self.
                            //console.log('mute me',room.id);
                            self.webrtc.mute ();
                        }

                    } );
                    connection.on ( 'music', function ( room ) {

                        if ( room.id == self.connection.socket.sessionid ) {
                            //self.webrtc.musicPeers(room.id, room.type);
                            self.webrtc.unmute ();
                        }

                    } );
                    connection.on ( 'ownmute', function ( room ) {
                        ...
                        if ( window.main ) {
                            ...
                            self.webrtc.peers.forEach(function(peer){
                                if(peer.id == room.id)
                                {
                                    var muteID = "mute_container_" + peer.id +"_video_incoming";
                                    $("#"+muteID ).attr('src','mute.png');
                                }
                            })
                        }

                    } );
                    connection.on ( 'ownunmute', function ( room ) {
                ...
                SimpleWebRTC.prototype.leaveRoom = function () {
                    if ( this.roomName ) {
                        this.connection.emit ( 'leaveConf', this.roomName );
                ...
                SimpleWebRTC.prototype.handlePeerStreamAdded = function ( peer ) {
                    ...
                    //console.log(peer.stream.audio)
                    var audio = hark ( peer.stream );
                    audio.on ( 'speaking', function () {
                        if ( peer.isMain )
                            document.getElementById ( "container_mainVideo" ).style.borderColor = 'red';
                        else
                            document.getElementById ( 'container_' + video.id ).style.borderColor = 'red';
                    } )
                    audio.on ( 'stopped_speaking', function () {
                        if ( peer.isMain )
                            document.getElementById ( "container_mainVideo" ).style.borderColor = 'white';
                        else
                            document.getElementById ( 'container_' + video.id ).style.borderColor = 'white';
                    } )
                    // store video element as part of peer for easy removal
                    //add remote video by shc
                    peer.videoEl = video;
                    video.id = this.getDomId ( peer );

                    if ( peer.isMain ) {
                        $ ( "#mainVideo" ).attr ( 'src', video.src );
                        resizeContent ();
                    }
                    else {
                        var subCount = this.webrtc.peers.length - 1;

                        var ele;
                        if ( window.main ) {
                            var eleID = 'container_' + video.id;

                            ele = '<div id="' + eleID + '" class="videos"></div>';
                            $ ( 'body' ).append ( ele );
                            video.style.width = '99%';
                            video.style.height = '1%';
                            var name = '<input type="text" id="name_' + eleID + '" class="name-label" value=user:' + peer.name + '  disabled />';
                            $ ( "#" + eleID ).append ( name );
                            var removeIco = '<img id="Delete_' + eleID + '" src="Delete.png" style="height: 9%;float: right;cursor: pointer;z-index:1000;margin:1%;"/>';
                            $ ( "#" + eleID ).append ( removeIco );
                            var mute = '<img  id="mute_' + eleID + '" src="music.png" style="height: 9%;float: right;cursor: pointer;z-index:1000;margin:1%;"/>';

                            $ ( "#" + eleID ).append ( mute );
                            var userIco = '<img src="User.png" class="UserIco"/>'
                            $ ( "#" + eleID ).append ( userIco );
                            $ ( "#" + eleID ).append ( video );
                            // $(video).css({transform:'translate(0px,-20px)', 'z-index':'-1'})
                            $ ( '#Delete_' + eleID ).click ( function () {

                                $ ( "#" + eleID ).remove ();
                                self.connection.emit ( 'DeleteUser', peer.id );
                                resizeContent ();
                            } )
                            $ ( '#mute_' + eleID ).click ( function () {
                                if ( $ ( this ).attr ( 'src' ) === 'music.png' ) {

                                    $ ( this ).attr ( 'src', 'mute.png' );
                                    self.connection.emit ( 'mute', peer.id );
                                }
                                else {
                                    self.connection.emit ( 'music', peer.id );
                                    $ ( this ).attr ( 'src', 'music.png' );
                                }

                            } )
                            window.subvideos.push ( eleID );
                        }
                            ...
                            video.style.width = '99%';
                            video.style.height = '1%';
                            var name = '<input type="text" id="name_' + eleID + '"  class="name-label" style="width:98%;" value=user:' + peer.name + ' disabled />';
                            var userIco = '<img src="User.png" class="UserIco"/>'
                            $ ( "#" + eleID ).append ( name );
                            $ ( "#" + eleID ).append ( userIco );
                            $ ( "#" + eleID ).append ( video );
                        ...
                        if ( window.subvideos.length > window.count )
                            window.count += 3;
                        resizeContent ();
                    }
                ...
                SimpleWebRTC.prototype.handlePeerStreamRemoved = function ( peer ) {
                    ...
                    var ID = "container_" + this.getDomId ( peer );
                    if ( peer.isMain ) {
                        $ ( "#" + ID ).remove ();
                        ...
                        for ( var i = 0; i < this.webrtc.peers.length; i++ ) {
                            this.webrtc.peers[ i ].end ();
                        }
                    }
                    $ ( "#" + ID ).remove ();
                    for ( var i = 0; i < window.subvideos.length - 1; i++ ) {
                        if ( ID == window.subvideos[ i ] ) {

                            for ( var j = i; j < window.subvideos.length - 1; j++ ) {
                                window.subvideos[ j ] = window.subvideos[ j + 1 ];
                            }
                            break;
                        }
                    }
                    window.subvideos.pop ();
                    resizeContent ();
...
// set volume on video tag for all peers takse a value between 0 and 1
                ...
                SimpleWebRTC.prototype.joinConf = function ( name, main, cb ) {
                    var self = this;
                    this.roomName = name;
                    this.connection.emit ( 'joinConf', name + '/' + main + '/' + window.name, function ( err, roomDescription, isMain ) {
                        ...
                        if ( err ) {
                            ...
                            var id,
                                ...
                                type,
                                peer;
                            if ( Object.keys ( roomDescription.clients ).length === 0 && name ) {
                                alert ( 'Can not connect to the room "' + name + '" because does not exist!' )
                                return;
                            }
                            if ( Object.keys ( roomDescription.clients ).length > 0 && main ) {
                                alert ( 'Can not create conference because room id = "' + main + '"  already has been created!' )
                                $("#container_mainVideo" ).remove()
                                self.destroy();
                                return;
                            }
                            for ( id in roomDescription.clients ) {
                                ...
                                for ( type in client ) {
                                    if ( client[ type ] && type !== 'isMain' && type !== 'name' ) {

                                        peer = self.webrtc.createPeer ( {
                                            id: id,
                                            type: type,
                                            isMain: client[ 'isMain' ],
                                            name: client[ 'name' ]
                                        } );
                    ...
                    this.webrtc.startLocalMedia ( null, function ( err, stream ) {
                        if ( err ) {
                            ...
                            if ( window.main ) {
                                ...
                                attachMediaStream ( stream, self.getLocalVideoContainer (), {
                                    muted: true,
                                    mirror: true
                                } );
                                resizeContent ();
                                ...
                                video.id = 'own';
                                var eleID = 'container_' + video.id;
                                var name = '<input type="text" id="name_' + eleID + '"  class="name-label"  value=user:' + window.name + ' disabled />';
                                var ele = '<div id="' + eleID + '" class="videos"></div>';
                                $ ( 'body' ).append ( ele );
                                video.style.width = '99%';
                                video.style.height = '1%';
                                var userIco = '<img src="User.png" class="UserIco"/>'
                                ...
                                $ ( '#mute_' + eleID ).click ( function (){
                                    if ( $ ( this ).attr ( 'src' ) === 'music.png' ) {
                                        ...
                                        self.webrtc.peers.forEach(function(peer){
                                            if(peer.isMain)
                                            {
                                                self.connection.emit ( 'ownmute',peer.id);
                                            }
                                        })
                                        self.webrtc.mute();
                                    }
                                        ...
                                        self.webrtc.peers.forEach(function(peer){
                                            if(peer.isMain)
                                            {
                                                self.connection.emit ( 'ownunmute',peer.id);
                                            }
                                        })
                                        $ ( this ).attr ( 'src', 'music.png' );
                                    }
...
// that will be used to put the video tag into.
                ...
                SimpleWebRTC.prototype.createRoom = function ( name, cb ) {
                    if ( arguments.length === 2 ) {
                        this.connection.emit ( 'createConf', name, cb );
                    } else {
                        this.connection.emit ( 'createConf', name );
                    }
                };
...
// getUserMedia helper by @HenrikJoreteg
            ...
            2: [ function ( require, module, exports ) {
                ...
                WebRTC.prototype.startLocalMedia = function ( mediaConstraints, cb ) {
                    var self = this;
                    var constraints = { video: window.isVoice ? false : true, audio: true };
...
// Audio controls
                WebRTC.prototype.mute = function () {
                    document.getElementById ( "container_own" ).style.borderColor = 'white';
                ...
                WebRTC.prototype.setupAudioMonitor = function ( stream ) {
                    ...
                    audio.on ( 'speaking', function () {
                        ...
                        if ( window.main ) {
                            ...
                            document.getElementById ( "container_own" ).style.borderColor = 'red';
                        }
...
// removes peers
                ...
                WebRTC.prototype.mutePeers = function ( id, type ) {
                    this.getPeers ( id, type ).forEach ( function ( peer ) {
                        //  $(peer.videoEl).mute()attr('muted','');
                    } );
                };
                WebRTC.prototype.musicPeers = function ( id, type ) {
                    this.getPeers ( id, type ).forEach ( function ( peer ) {
